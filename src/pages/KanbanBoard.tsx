...
// ── helpers: แปลง server → รูปแบบที่ UI ใช้
function adaptTask(t: any): Task {
  return {
    id: String(t.id),
    title: t.title ?? '',
    description: t.description ?? null,          // ✅ ใส่ description ให้ตรง type
    // ด้านล่าง optional ถ้ามีใน type คุณ
...
export default function KanbanBoard() {
  ...
  const addTask = async (colId: string, title: string) => {
    if (!board) return;
    if (!user?.id) { setError('ยังไม่ได้ล็อกอิน'); return; } // ✅ created_by NOT NULL
    try {
      const payload = {
